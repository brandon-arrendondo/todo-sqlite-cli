...
fn v5_database_migrates_to_v7_on_open() {
    ...
    // Pre-existing rows default location and implementation_client to NULL.
    ...
    let implementation_client: Option<String> = conn
        .query_row(
            "SELECT implementation_client FROM tasks WHERE title = 'a'",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
// Frozen copy of the v6 schema (adds `location` + `related` over v5, no
// `implementation_client` column yet) — intentionally not derived from
// `db.rs`'s current schema, so this fixture stays historically accurate as
// the schema keeps evolving.
...
fn v6_database_migrates_to_v7_on_open() {
...

    {
        ...
        .unwrap();
        // implementation_client must NOT exist yet on a v6 DB.
        conn.query_row("SELECT implementation_client FROM tasks", [], |r| {
            r.get::<_, Option<String>>(0)
        })
        .expect_err("implementation_client column must NOT exist before migration");
    }
